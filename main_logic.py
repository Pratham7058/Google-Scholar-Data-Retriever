...
def scrape_scholar_data(search_query, num_results=10, max_retries=3):
    """
    Scrape publication data from Google Scholar without using a proxy
    """
    publications_data = []
    ...
    for attempt in range(max_retries):
        try:
            ...
            for pub in author['publications'][:num_results]:
                try:
...
def main():
    ...
    num_results = int(input("Enter number of results to retrieve (default 10): ") or 10)
    
    print("Scraping data from Google Scholar...")
    publications_data = scrape_scholar_data(search_query, num_results)
    
    if publications_data:
        print(f"\nFound {len(publications_data)} publications")
        save_to_excel(publications_data)
    else:
